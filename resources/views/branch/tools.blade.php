...
@section('content')
    <div id="content-container">
        <div id="page-head">
            ...
            <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
            <div id="page-title">
                <h1 class="page-header text-overflow">Herramientas Ministeriales</h1>
            </div>
            <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
            <!--End page title-->
            <!--Breadcrumb-->
        ...
        <div id="page-content">
            ...
            <div class="panel" style="background-color: #e8ddd3;">
                <div class="panel-heading">
                    <h3 class="panel-title text-center">Tipos</h3>
                </div>
                <div class="row">
                    <div class="col-md-6">
                        <div class="pad-all">
                            <form id="collection_type_form" method="POST" action="{{ route('branch.toolsPost') }}">
                                @csrf
                                <label>Crear tipo de Donativo</label>
                                ...
                                <input style="border:1px solid #ddd; padding:7px;outline:none" name="name" type=text
                                    pattern="[a-zA-Z0-9_ -]+" title="Solo letras, numeros, _ y - son aceptados"
                                    Placeholder="Nombre del Donativo" required />
                                <button type="submit" class="btn btn-success btn-md"><i class="fa fa-plus"></i> Crear</button>
                            </form>
                        </div>
                        <div class="pad-all">
                    ...
                    <div class="col-md-6">
                        <div class="pad-all">
                            <form id="service_type_form" method="POST" action="{{ route('branch.toolsPost') }}">
                                @csrf
                                <label>Crear tipo de Servicio</label>
                                <input type=text name=branch_id value="{{ \Auth::user()->branchcode }}" hidden=hidden />
                                <input type=text name=s_type_c value="{{ \Auth::user()->branchcode }}" hidden=hidden />
                                <input style="border:1px solid #ddd; padding:7px;outline:none" name="name" type=text
                                    Placeholder="Nombre del Servicio" type=text pattern="[a-zA-Z0-9_ -]+"
                                    title="Solo letras, numeros, _ y - son aceptados" required />
                                <button type="submit" class="btn btn-success btn-md"><i class="fa fa-plus"></i> Crear
                                    </button>
                            </form>
                        </div>
                        <div class="pad-all">
...
@section('js')
    ...
    <script>
        $(document).ready(() => {
            $.fn.dataTable.ext.errMode = () => alert('Error al cargar el contenido, por favor recarga la página');
